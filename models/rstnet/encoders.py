...
class EncoderLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, relative_geometry_weights, attention_mask=None, attention_weights=None, pos=None):
        ...
        q = queries + pos #50*49*512
        k = keys + pos #50*49*512
...
class MultiLevelEncoder(nn.Module):
    ...
    def forward(self, input, attention_weights=None, pos=None):
        ...
        relative_geometry_embeddings = BoxRelationalEmbedding(input)#50*49*49*64
        flatten_relative_geometry_embeddings = relative_geometry_embeddings.view(-1, 64)
        box_size_per_head = list(relative_geometry_embeddings.shape[:3])#50*49*49
        box_size_per_head.insert(1, 1)##50*1*49*49
        ...
        relative_geometry_weights = torch.cat((relative_geometry_weights_per_head), 1) ##50*8*49*49
