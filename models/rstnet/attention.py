...
class ScaledDotProductGeometryAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, box_relation_embed_matrix, attention_mask=None, attention_weights=None):
        ...
        w_g = box_relation_embed_matrix #50*8*49*49
        w_a = att
        ...
        out = self.fc_o(out)  # (b_s, nq, d_model) 50*49*512
        return out
...
class ScaledDotProductAdaptiveAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, language_feature=None):
        ...
        b_s, nq = queries.shape[:2] #50 22 512
        nk = keys.shape[1] #49
        ...
        language_att = torch.cat([language_att[:, :, i, i].unsqueeze(-1) for i in range(nq)], -1)#50 *8 *22

        # 融合att
        combined_att = torch.cat([att, language_att.unsqueeze(-1)], -1)     # (b_s, h, nq, nk + 1) 20 8 22 50
...
class MultiHeadAdaptiveAttention(Module):
    '''
    Multi-head attention layer with Dropout and Layer Normalization.
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
                 attention_module=None, attention_module_kwargs=None, comment=None):
        super(MultiHeadAdaptiveAttention, self).__init__()
        self.identity_map_reordering = identity_map_reordering

        self.attention = ScaledDotProductAdaptiveAttention(d_model=d_model, d_k=d_k, d_v=d_v, h=h, comment=comment)
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, language_feature=None):
        ...
        if self.identity_map_reordering:
            ...
            k_norm = self.layer_norm(keys)
            v_norm = self.layer_norm(values)
            s_norm = self.layer_norm(language_feature)

            out = self.attention(q_norm, k_norm, v_norm, attention_mask, attention_weights, s_norm)
            out = queries + self.dropout(torch.relu(out))
        else:
            out = self.attention(queries, keys, values, attention_mask, attention_weights, language_feature)
            out = self.dropout(out)
            out = self.layer_norm(queries + out)
        return out
