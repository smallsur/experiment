...
class DecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att, pos):
        ...
        # MHA+AddNorm
        key = enc_output + pos
        enc_att = self.enc_att(self_att, key, enc_output, mask_enc_att)#50*22*512 50*49*512 en
...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, input, encoder_output, mask_encoder, pos):#pos 50,49,512
        # input (b_s, seq_len)
        b_s, seq_len = input.shape[:2]#50,22
        mask_queries = (input != self.padding_idx).unsqueeze(-1).float()  # 50*22*1
        mask_self_attention = torch.triu(torch.ones((seq_len, seq_len), dtype=torch.uint8, device=input.device),
                                         diagonal=1)
        mask_self_attention = mask_self_attention.unsqueeze(0).unsqueeze(0)  # (1, 1, 22, 22)
        mask_self_attention = mask_self_attention + (input == self.padding_idx).unsqueeze(1).unsqueeze(1).byte()
        mask_self_attention = mask_self_attention.gt(0)  # (b_s, 1, seq_len, seq_len) fasle能看见，权值左乘value
