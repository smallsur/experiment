...
class BeamSearch(object):
    ...
    def apply(self, visual: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        self.seq_mask = torch.ones((self.b_s, self.beam_size, 1), device=self.device)#是否结束
        self.seq_logprob = torch.zeros((self.b_s, 1, 1), device=self.device)#每条句子的概率
        self.log_probs = []
        self.selected_words = None
        if return_probs:
            self.all_log_probs = []
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        ...
        selected_beam = (selected_idx / candidate_logprob.shape[-1]).to(torch.int64)
        selected_words = selected_idx - selected_beam * candidate_logprob.shape[-1]

        self.model.apply_to_states(self._expand_state(selected_beam, cur_beam_size))#batch_size
